...
9. 最終回覆包含：
   ...
   - commit SHA
   - 尚未解決或刻意未處理的問題
10. 使用者要求「更新處理卡」時，交付包含提交、推送及核對線上發布內容；不以本機修改作為完成。工作樹異常或混有其他變更時，從遠端最新版在乾淨工作樹只提交本次檔案。

## 內容慣例

### 獨立車險處理卡
- 使用者要求：車險處理卡的更正或後續操作建議，預設同一輪同步更新 `parking-incident-guide.html`，不可只在對話中提供；只有使用者明確說「純討論／不修改」時才不動檔案。實質更新仍須提交、推送並核對公開頁面。
- 卡片須區分已送出、待回覆、未寄出的建議稿與未授權施工；新舊事故分案。以佳蓉最大利益為目標，但僅用可核實事實，不承認未能確認的既有損傷，也不宣稱未證實的事故因果。
...
- 獨立停車事故卡如列出車廠估價，應讓使用者直接看到維修部位、逐項與總金額，同時標示照片初估、保險公司核准、實車檢查及施工授權各自的狀態。原估價 PDF 含私人識別資料，僅留在 `private/`，公開頁只摘要維修項目與費用，不放原件連結。
- 獨立停車事故卡的線上理賠引導應以當前事故的可核對事實填寫：逐步列出表單選項、已知值、需當事人核對的欄位、完整可複製英文事故敘述及逐段中文翻譯；已過期限或未確認完成的動作不得寫成將於期限內完成。不同日期的事故分開建案，前案的自負額書面答覆只作參考，須請保險公司另行確認新案。
- MAS汽車保單「One event – one excess」表示單一事件只適用一次自負額，不應僅因同一事故同時有自車損失及第三方責任，就按車輛或損失類別拆算兩筆；適用總額仍須核對保單附表與任何附加自負額。先前案件的個別豁免回信不可當作新案保證，應向MAS書面確認新案處理方式。
- 處理卡各事故依當下必要性排列：目前要做的保險／車主處理先列，條件式備用程序（如 Police 105）移到主要待辦之後，明確標示觸發條件；若目前沒有觸發條件，不可讓備用程序看起來像必辦或當前待辦。
- 車險處理卡提及修理廠時，需如實表達是保戶提出的偏好或選擇，不要寫成保險公司指示；若同一修理廠處理不同事故，仍要求分案檢查與分開估價，且把檢查／估價和授權施工分清楚。
...
- 2026-09-22 MAS Assessing 就舊案 801516／Rosiez estimate 14867 詢問左前葉子板是否屬於 8/12 事故，因其依事故描述與照片認為本案相關損傷看似限於左前保險桿。轉寄原信顯示時間為 9/22 10:08，早於同日 16:10 的 BMW 新事故；該新事故與 9/21 已送審的舊估價不得混為一談。回覆須依駕駛記憶與同期照片據實說明；資訊不足時清楚說明無法確認，不得臆測，也不可把 MAS 的初步判斷說成當事人已確認的事實。
- 2026-09-25 MAS 對 801516 的最新回信將左前葉子板描述為既有、看起來與 8/12 事故無關，並表示暫不納入；這是 MAS 依其掌握資料作的判斷，不是照片已證明損傷時間。首次申報照片未清楚、完整呈現整片葉子板；佳蓉 9/16 另寄給 Rosiez 的照片有副本給 MAS，但目前未知評估員是否檢視。回覆不可同意只審保險桿，也不可把葉子板說成確定舊傷或確定由事故造成；應詢問 9/16 照片是否納入、要求指出判斷所據的具體照片／可見特徵，並請保留葉子板評估範圍。只確認審查，不授權施工；取得完整書面範圍與金額後再決定是否修自車。與 9/22 BMW 新案分開。
- 2026-09-25 MAS 新案 Section 5「Review and submit」核對：`Were other vehicles involved?` 問的是受保車以外有無其他車輛參與；只有受保車與一台對方車時仍選 Yes，只有沒有其他車輛時才選 No。使用者提供的審核頁目前顯示 No，需改 Yes；事故敘述亦有重複段落，應先清空欄位、一次貼入處理卡單一完整版本，回摘要檢查無重複，再確認聲明真實後送出。截圖所示其他欄位為駕駛 Me、駕照 Overseas／1 year；酒精、drugs、DUI 檢測及前科皆選 No，但每題均為不同事實，逐題按實況核對，不以「沒有違規」推定是否飲酒、用藥或受過檢測。佳蓉另明確確認本次未服藥及無相關駕駛違規／定罪，因此 medication 與 previous driving convictions 填 No。審核頁不等於已申報，未見 MAS 確認頁前不得標記為已送出。
- 新事故若與先前未修車損在同一鈑件，不必在事故敘述重述舊事故全貌或主動列舊案號；但須簡潔揭露事發前已存在且未修復的損傷，請 MAS 依日期照片分別判定本次與既有損傷及維修範圍。不能因事故原因不同就省略相關既有損傷；避免誇大為必然拒賠，依現行 MAS 條款既有車損不在自車損失承保範圍，理賠時並須誠實配合及提供核賠所需資料。
- 2026-09-25 較新的 MAS 9/22 新案送件副本取代送件前草稿：使用者已提交理賠；實際事故敘述沒有上項建議的舊案段落，附件只有本次事故照片。副本確認「其他車輛」為 Yes、敘述不重複、使用顧客停車場；PDF 第 1 頁事故時間誤列 8:10 p.m.，第 2 頁敘述為正確 4:10 pm，需寄 claims@mas.co.nz 書面更正。確認信只表示已收到並將審查保障，主旨數字亦不可自行認定為正式新案號。處理卡須記錄實際送件內容，不可把原建議段落寫成已提交；舊案資料和舊照先留存，MAS 問到重疊損傷時據實提供，不得將既有損傷申報為新損。新案已送後可回覆舊案待回信，不需等正式新案號。
- 2026-09-26 已在 Gmail 核對更正信寄件副本：9/26 09:27（NZST）寄至 `claims@mas.co.nz`，要求將 9/22 新案 PDF 首頁時間由 8:10 p.m. 更正為 4:10 p.m.，並請 MAS 確認與案件 801516 分開的新案正式案號。寄件副本只證明信已寄出，不代表 MAS 已回覆、完成更正或核准理賠；獨立處理卡應標示「更正信已寄，等待 MAS 書面確認」，主手冊與 `index.html` 不連動修改。
- 2026-09-26 更正此前舊案回信策略：不得以「只確認繼續審查保險桿」默認或接受葉子板排除。`parking-incident-guide.html` 已更新為詢問 MAS 是否檢視 9/16 補交照片、要求說明排除判斷依據、保留葉子板評估範圍；不確認損傷時間、不主張必然由 8/12 事故造成、不授權施工。建議信尚未寄出，只談 801516，不提 9/22 新案或與爭點無關的估價細節。舊案自負額條件以 MAS 8/17 書面答覆為準，不能外推至新案。
- 佳蓉已確認 2026-09-22 BMW 事故發生時未服藥，且沒有相關駕駛違規／定罪，故 medication 與 previous driving convictions 填 No。酒精、毒品與 DUI 是否受檢是分開的事實題，不得從「沒有違規」推定；按本人實況核對。事故地點為顧客停車場，不是員工停車場；英文敘述使用 `customer car park`。
...
- 2026-09-20 `parking-incident-guide.html`新增現場評估口語 QA，採英文短句緊接中文對照，涵蓋檢查目的、事故物理事實、前保險桿與感測器／固定座不自行判定、評估／出車／交通費、責任與第三方問題、維修授權、文件簽署、MAS案件編號及要求改用Email；維持未書面確認費用、未取得MAS處理前不在現場同意維修、付款或和解。主手冊不連動修改。
- 2026-09-24 使用者確認 Rosiez estimate 14867 僅依照片製作，現場檢查尚未完成，佳蓉因工作尚未約時間。MAS 9/22 詢問左前葉子板與 8/12 舊案的關聯；事故卡應先回覆照片估價與未現場檢查的事實，詢問 MAS 是否需要現場評估、由誰安排，並沿用 MAS 已承擔評估費的書面確認，但先查明是否另有出車／交通／現場費。收到指示及確認工作班表後再約；不得臆測葉子板因果或授權維修。主手冊不連動修改。
- 2026-09-21 後續釐清信另要求 Rosiez 提供已送保險公司的 estimate 14867 副本，或至少提供逐項修理範圍與總額；頁面明確標示索取文件僅供核對，不代表接受金額或授權維修。主手冊不連動修改。
- 2026-09-24 使用者要求 8/12 Mazda 舊案對 MAS 的回信範本與目前操作段落先不提其他事故；事故卡應避免把未被詢問的其他案件細節帶入本案對外訊息。若 MAS 後續明確詢問，再依使用者確認及可核實資料處理。其他事故分頁及其事實紀錄不因此改動。

- 保險溝通以佳蓉最大利益為原則，但所有陳述必須真實、可核實；不得隱瞞保險公司明確詢問或理賠審查必要的重大事實，也不主動擴張到未詢問且無助推進案件的細節。避免不必要的承認、技術爭辯、接受最終範圍或放棄維修選項。修理廠負責說明維修工法、估價項目與技術必要性；保險公司負責理賠範圍、核定金額、自負額與流程。任何對外草稿需確認收件人、案件及授權範圍；同意估價審查不等於授權施工，也不等於接受最終理賠範圍或決定是否維修。

## 已知注意事項

- 2026-09-23 BMW 車主已回覆紙條並提供姓名與電話，使用者確認彼此是同店員工且可中文溝通。事故卡應將直接交換資料列為首要步驟，要求在48小時內以文字給出駕駛姓名、住址、電子地址（有Email時）、車牌及事故地點；車主目前可聯絡時，依《Land Transport Act 1998》第22(5)條及 Police 指引，60小時警方通報是車主無法找到／聯絡時的條件式步驟，卡片不得沿用舊的「仍未找到車主」文案。公共頁不列車主私人電話及姓名。
...
- 2026-09-24 更新獨立事故卡時，佳蓉確認 8/12 舊案的左前葉子板問題已回覆 MAS；舊案是否修復佳蓉自車尚未決定，任何較早「不修」措辭均不得當成她已作決定。9/22 新案則等 MAS 書面評估、維修範圍及費用後再決定是否修。照片顯示 Tiida 左前保險桿有擦痕、靠輪拱處接縫脫開，並有一件狹長塑膠件被單獨拍下；這表示需檢查接合／固定處，但無法由影像識別該件確切功能、判定固定件何時損壞或斷裂，亦不能判定整支保險桿必須更換。BMW 後保險桿車牌下方可見局部擦痕，照片未顯示明顯大範圍變形，但刮痕深度及隱藏損傷待實檢。8/12 影像亦有前保險桿擦痕，estimate 14867 列前保險桿修復、烤漆及左側固定座，均未修；須交標日期前後照片由 MAS／維修廠分列新損、未修舊損與必要工項，不自行推定擦痕重疊或因果。不得寫「通常全額」或無來源的 NZD 50–100 預估。MAS「同一事故一筆自負額」須與舊損／betterment 分攤區分，後者可能另計；目前資料所示標準自負額 NZD 500，仍要 MAS 書面確認該新案金額與附加額。申報文字以可見物理事實為主，修理決定與費用問題放在後續書面補充。舊案歷史回覆草稿須明確封存，不能顯示為待辦。主手冊不連動修改。
