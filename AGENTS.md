...
- 目前 Mastercard 匯率基準：`08/03`
  - `1 NZD = 19.1668709 TWD`
  - `1 MYR = 7.9670235 TWD`
  - `1 USD = 32.4257820 TWD`
- 目前財務摘要：
  - 已支出：`TWD 289,778`
  - 待支出：`TWD 16,258`
  - 總預估：`TWD 306,036`
- 目前 `src/sections/` 中已有導航總表、景點票券與執行時間、住宿與付款總表等核心區塊的源碼。
- 根目錄另有獨立的 `medical-consult.html` 線上問診整備頁；此頁不連入主手冊、不由 `build.py` 組裝，並以少量必填欄位產生可一次複製的英文問診說帖。修改獨立頁時不得連動改寫 `index.html` 或主手冊封面更新時間，除非使用者另行要求。
