...
- 目前 JPY 金額採信用卡各月帳單實際成本；不再維護 MYR 每日匯率，歷史 MYR 消費仍保留原幣與正式 TWD。
- 目前結案匯率基準：Mastercard `08/13`；JPY 採各月帳單實際成本
  - `1 NZD = 18.8347111 TWD`
  - `1 USD = 32.0700146 TWD`
  - 6月 JPY：`1 JPY = 0.19701 TWD`
  - 7月 JPY：`1 JPY = 0.20094 TWD`
  - 8月 JPY：`1 JPY = 0.20109 TWD`
- 目前財務摘要：
  - 實際支出：`TWD 305,309`
  - 39日兩人旅行成本：`TWD 189,095`
  - 佳蓉長住成本：`TWD 27,653`
  - 資產／長期持有：`TWD 88,561`
- 成本歸屬採三層：購車及購車時修理／驗車、MAS與自有車行政列「資產／長期持有」；WHV、Working Holiday保險、留下的年約門號、保暖衣物、家用品、藥品、清潔衛生用品與車輛小工具列「佳蓉長住成本」；交通、住宿、餐飲、票券、伴手禮、旅遊保險、律德NZeTA／IVL、雙人國際駕照及貓旅館列「39日旅行成本」。
- 佳蓉WHV沿用先前對話確認的舊總帳歷史值TWD7,169，不以目前新制NZD770回溯重算；雙人國際駕照依公定規費每人TWD250、合計TWD500。律德NZeTA NZD17與律德IVL NZD100均為律德本人費用。
- 私人收據集中於`private/receipts/`並受`.gitignore`排除；公開手冊只保留商家、原幣與成本拆分，不公開影像、卡號末碼或其他私人憑證內容。
- 財務修改前應直接抓取`https://gpt.greenparty.org.tw/nz-handbook/`核對線上文字與總額；公開網站已部署內容是財務基準，本機工作樹與先前對話只作佐證，不得用較舊本機數字覆寫線上最新版。
...
- `parking-incident-guide.html` 應依當下必要性排序，不按機構分類：目前採「最新狀態、一眼進度、安排估價檢查、把MAS正式案號傳給車主、書面補件、條件式105、金額與責任」；已完成的送件及更正內容收進可展開歷史區，避免擋住當前行動。
- 車主後續提供保險理賠管道與案件編號時，教戰卡應將「索取對方保險資料」標為已完成，下一則訊息只需確認收到並補齊尚缺資料；對方案件資料應直接納入MAS送件及證據清單，不再重複詢問。
- 當完整資料已傳給車主且MAS已明確提供正式案件編號時，只再傳一則MAS案號供車主轉交其保險公司；不得重傳全部個資或把先前文件識別碼誤當正式案件編號。
...
- 2026-08-13 本案MAS線上理賠已送出；MAS當日Email表示會先確認保障範圍並在24小時內聯絡（星期五、週末或假日則下週初），問題應寄`claims@mas.co.nz`，不可回覆no-reply信。隨附PDF首頁事故時間誤列6:35 p.m.，但第2頁英文敘述正確為2:35 pm；停車事故卡必須把Email更正紀錄排為第一優先，不得重送理賠。PDF上的`326147`未明示為MAS案件編號，未獲MAS明確確認前不得傳給Gerald稱為claim reference。
- 2026-08-13 已確認事故時間更正Email於18:23（NZST）送至MAS Claims，內容正確要求將PDF首頁6:35 p.m.改為2:35 p.m.；卡片此後改以等待MAS書面確認、24小時後才追蹤及僅書面補件為順序。等待與補件段落的每個英文話術必須緊接簡潔中文翻譯。
- 停車事故卡的「一眼確認進度」不是頁尾總結，必須放在「目前狀態」後方；其三欄內容需同步呈現已完成、等待門檻與不要做，不能保留過時的立即送件指示。
- 2026-08-14 MAS已書面確認本案接受、事故時間更正為2:35 pm、續保保費不受本案影響，並表示NZD500自負額通常於修復完成時支付給維修廠。MAS保單另明定同一事件只適用一次自負額；教戰卡應請MAS安排或推薦靠近Upper Riccarton／Peer Street的認可維修廠，只安排檢查／估價、不先授權開工，並先書面確認檢查／估價費是否由MAS承擔、是否另計，再確認若不修自車時的自負額付款安排。照片只能支持「無明顯立即停駛跡象」，不能取代固定座、裂痕及安全性的實車檢查。
- 停車事故卡回覆MAS的窗口應同時寄送：`assessors@mas.co.nz`（Lene，負責維修廠／Request for a Quote）並副本`claims@mas.co.nz`（Claims Adviser，負責自負額與理賠問題）；不要回覆Clear21轉寄地址。
...
- 2026-08-14 Mastercard Global直連仍回403；SG Rates的NZD、USD、TWD三頁MasterCard牌告日一致為08/13，交叉匯率更新為NZD/TWD18.8347111、USD/TWD32.0700146。永豐JPY即期賣出更新為08/13 15:30的0.2036。未查詢MYR，只重算日圓帳單估值與結案預算換算；正式TWD不覆寫。財務更新為實際支出TWD305,574、旅行淨支出TWD217,153。
- 2026-09-03 完成首輪全站閱讀體驗與視覺優化：加入純 CSS 浮動回頂鈕與平滑滾動，各章節尾加入「回到目錄」連結；首頁目錄新增分類主題徽章；作戰卡時段標籤底色 Badge 化、Plan B 邊框升級；財務折疊卡片互動強化；A4 列印加入精準防截斷與精確色彩控制；網頁更新時間同步更新。
- `.DS_Store`、`tmp/`、本機查信／修補腳本與根目錄身分文件均由 `.gitignore` 排除，不得納入公開提交。
- 2026-09-04 全站稽核後同步目前財務摘要為TWD305,309（旅行TWD189,095、長住TWD27,653、資產TWD88,561），住宿表永豐幣倍9筆本金修正為TWD15,414；公開主手冊移除私人基地地址與Bookme訂單碼，地圖以Westfield Riccarton公開地標代替；版本庫清除誤納的`.DS_Store`與部署測試檔。
- 2026-09-16 Rosiez Frikkie 再次要求車輛登記、四角與受損處照片，以判斷能否直接依照片製作估價；佳蓉已於紐西蘭時間16:44寄出7張照片，副本MAS Assessors與MAS Claims，並明確限定只授權評估與估價、尚未授權維修。停車事故卡目前應顯示「照片已寄出，等待Rosiez判斷」，不得再要求重寄或把估價案件識別14867寫成金額。
- 2026-09-17 Rosiez已確認照片不足以準確估價，必須到現場檢查車輛並請佳蓉提供日期與時間。使用者更正案件卡範圍：車險卡不放內部iOS訊息或傳送按鈕；使用者可另以本機訊息私下提醒佳蓉，但案件卡要保留佳蓉下一步以原Email信串回覆Rosiez的完整信件文字，指定2/32A Peer Street, Upper Riccarton, Christchurch 8041為評估地址，並要求先書面確認assessment、call-out、travel及其他現場費用。完整私人電話僅留在獨立事故卡，不擴散至主手冊或AGENTS.md。
...
### 2026-09-03（全書戰術執行命令與備案還原升級）
- 變更性質：**修正幻覺，精確還原「實際執行紀錄（Operational Orders）」**。
- 執行內容：
  - 徹底糾正前次代理人將「教戰手冊」誤以為「遊記流水帳」的幻覺錯誤。手冊雖為旅程結束後的留存紀錄，但其核心價值在於「事前規劃紀律」與「風險控管規則」。
  - 完整讀取 Gmail 的 30 封「紐西蘭明日行程提醒」信件（即真實現地發布的次日戰術執行命令）。
  - 將 07/20 至 07/31 的行程插槽（slots），嚴格依據信件原文中的時間硬節點（Hard Nodes）、行動準則（如：不跨鐵路、不涉過膝水）、條件限制（如：17:15前入座、每人至少點一杯飲料）重新寫入 `10-south.html`。
  - 刻意保留所有 `<div class="planb">`（備案與取消條件），不為追求語境和諧而抹去這些展現紀律與風險意識的關鍵邏輯。
  - 將前次被錯誤竄改的檔案（00-cover, 01-finance, 02-tickets, 04-stay, 05-klia, 06-prep, 11-aurora, build.py）全部還原至未受污染的基準版本（9ba4aef），以確保數字紀錄與原始狀態維持 100% 精準。

### 2026-09-03（全書結構與排版品質大清洗）
- 變更性質：**一致性修正與北島／基督城語氣還原**。
- 執行內容：
  - **語氣與內容升級**：將北島（08-north）作戰卡完全依照 Gmail（00_mail 到 07_mail）的戰術細節覆寫，恢復嚴格的時間節點與刪減規則（Plan B）；將基督城（09-chch）手冊中錯誤留存的事後字眼（已成交、實際備註）改回嚴謹的事前防禦語氣。
  - **目錄與標題同步**：修正 `00-cover` 的 6 處 TOC 連結，使其與各章節 `<h1>` 完全字面一致。
  - **財務與住宿資料對齊**：修正 `01-finance` 標題階層錯亂（h1 直跳 h3），並將 `04-stay` 的 8 月日圓匯率強制同步為 `0.20109`（商業住宿小計 TWD 34,946），消除兩表的本金差異。
  - **符號與隱私洗淨**：全書撤除 `10-south` 遺漏的租車與票券平台真實訂單號碼；統一彎引號 `’` 為標準直引號 `'`；統一跳脫 `&amp;` 實體；拔除 `02-tickets` 中不合常理的小數點尾數 `375.58`。

### 2026-09-03（台新09月電子帳單入帳與核銷）
- 變更性質：**正式帳單核對完成**。
- 執行內容：
  - 由使用者 Gmail 下載並解密台新 2026 年 09 月信用卡電子帳單（`TSB_Creditcard_Estatement_202609_unlocked.pdf`，存放於 `private/billing/` 並受 `.gitignore` 保護）。
  - 核對 08/12 交易：MAS 自有車全險 NZD 552.08 正式實刷本金 TWD 10,500（海外交易手續費 TWD 158）、Skinny 門號續期 NZD 9 正式實刷本金 TWD 171（海外交易手續費 TWD 3），兩筆交易與手冊既有登載之實刷本金完全吻合。
  - 同步更新 `01-finance.html` 稽核表為「台新06－09月」，並更新封面網頁更新時間戳記至 18:58。
